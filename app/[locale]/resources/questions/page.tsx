...
export default function QuestionsPage() {
  ...
  return (
    <main className="relative z-10 py-12 sm:py-20">
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(getCollectionPageJsonLd(t("jsonLd.title"), t("jsonLd.description"), `${BASE_URL}/${locale}/resources/questions`, locale)) }} />
      {items.map((item) => (
        <script key={item.id} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(getLearningResourceJsonLd(item, locale)) }} />
      ))}

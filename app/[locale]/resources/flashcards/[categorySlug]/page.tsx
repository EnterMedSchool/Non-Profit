...
export async function generateMetadata({
  ...
  const cardCount = decks.reduce((sum, d) => sum + d.cardCount, 0);
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `${BASE_URL}/${locale}/resources/flashcards/${categorySlug}`,
      type: "website",
      images: [{ url: ogImagePath("resources", "flashcards", categorySlug), width: 1200, height: 630 }],
    },
    alternates: {
      canonical: `${BASE_URL}/${locale}/resources/flashcards/${categorySlug}`,
    },
  };
}
    ...
    <main className="relative z-10 py-12 sm:py-20">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "CollectionPage",
            name: `${category.name} - Flashcard Decks`,
            description:
              category.description ??
              `Flashcard decks for ${category.name}`,
            url: `${BASE_URL}/${locale}/resources/flashcards/${categorySlug}`,
            inLanguage: locale,
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <PageHero
          ...
          annotation={`${t("category.cardsCount", { count: cardCount })} · ${t("category.decksCount", { count: decks.length })}`}
        />
            ...
            <div className="mt-8">
              ...
              <div className="flex flex-wrap gap-2">
                {childCategories.map((child) => (
                  <Link
                    key={child.id}
                    href={`/${locale}/resources/flashcards/${child.slug}`}
      ...
      >
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          >
            ...
            {t("category.cardsCount", { count: deck.cardCount })}
          </span>
          {difficultyKey && (
            <span
          ...
          {deck.estimatedMinutes != null && (
            <span
              className={`inline-flex items-center gap-1 rounded-lg ${colors.bg} ${colors.text} px-2.5 py-1 text-xs font-semibold border border-current/20`}
            >
